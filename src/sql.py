...
#Idle Warehouse Periods
idle_warehouse_periods_query = """
SELECT
    WAREHOUSE_NAME,
    COUNT(*) AS IDLE_PERIODS,
    SUM(TIMESTAMPDIFF(SECOND, RESUMED_AT, SUSPENDED_AT)) AS IDLE_DURATION_SECS
FROM
    SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_EVENTS_HISTORY
WHERE
    RESUMED_AT IS NOT NULL
...
GROUP BY
    WAREHOUSE_NAME
ORDER BY
    IDLE_DURATION_SECS DESC;
"""

#Warehouse Credit Usage by User
warehouse_credit_usage_by_user_query = """
SELECT
    WAREHOUSE_NAME,
    USER_NAME,
    SUM(CREDITS_USED) AS TOTAL_CREDITS_USED,
...
top_users_by_query_volume_query = """
SELECT
    USER_NAME,
...
#Inactive Warehouses
inactive_warehouses_query = """
SELECT
    WAREHOUSE_NAME,
    MAX(SUSPENDED_AT) AS LAST_SUSPEND_TIME
FROM
    SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_EVENTS_HISTORY
WHERE
...
GROUP BY
    WAREHOUSE_NAME
ORDER BY
    LAST_SUSPEND_TIME DESC;
"""

#Query Retry Events
query_retry_events_query = """
SELECT
    QUERY_ID,
    WAREHOUSE_NAME,
    USER_NAME,
...
#Warehouse Auto-Suspend Configurations
warehouse_auto_suspend_configurations_query = """
SELECT
    WAREHOUSE_NAME,
    AUTO_SUSPEND,
    AUTO_RESUME,
...
#Warehouse Cache Efficiency
warehouse_cache_efficiency_query = """
SELECT
    WAREHOUSE_NAME,
    COUNT(*) AS CACHE_HIT_QUERIES,
    SUM(CREDITS_USED) AS TOTAL_CREDITS_USED
FROM
    SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY
WHERE
    START_TIME >= DATEADD(DAY, -30, CURRENT_TIMESTAMP)  -- last 30 days
    AND QUERY_TEXT LIKE '%CACHE_HIT%'
GROUP BY
    WAREHOUSE_NAME
ORDER BY
    CACHE_HIT_QUERIES DESC;
"""
